...
def buy():
    ...
    if request.method == "POST":
        ...
        # Update user's cash and insert transaction into history
        db.execute("UPDATE users SET cash = cash - :purchase WHERE id = :user_id",
                   purchase=quote["price"] * int(shares), user_id=session["user_id"])
        db.execute("INSERT INTO history (user_id, symbol, shares, price, transacted, date) VALUES (:user_id, :symbol, :shares, :price, 'BUY', datetime('now'))",
                   user_id=session["user_id"], symbol=quote["symbol"], shares=int(shares), price=quote["price"])
...
def sell():
    ...
    if request.method == "POST":
        ...
        # Calcula o custo total das ações a serem vendidas
        total_cost = shares * stock_info["price"]

        # Verifica o valor de total_cost
        print(total_cost)

        cash = db.execute("SELECT cash FROM users WHERE id = :user_id", user_id=session["user_id"])[0]["cash"]

        if cash < total_cost:
            return apology("You do not have enough cash to complete this transaction", 400)

        # Atualiza o saldo de caixa do usuário e registra a transação de venda no histórico
        ...
        # Teste da função usd() para ver se formata corretamente o valor monetário
        print(usd(total_cost))

        # Renderiza o template de sucesso da venda
        return render_template("sell_success.html", total_cost=usd(total_cost))

    else:
        # Se o método não for POST, renderiza a página de venda com as ações disponíveis
